        ...
        <div class="card-body">
            <div class="tabs-container">
                <div class="tab-content" id="tab1-content" style="display: block;">
                    @include('includes.special_wholesale')
                </div>
                ...
                <tbody>
                    @foreach ($sales as $index => $sale)
                        <tr>
                            ...
                            @endphp
                            <td class="">{{Illuminate\Support\Carbon::parse($sale->date)->format('D, d M Y \a\t H:i:s') }} </td>
                            <td>{{ $sale->seller }}</td>
                        </tr>
                    @endforeach
                </tbody>
...
@section('scripts')
    ...
    <script>
        $('#cart-container').on('click', '.remove-from-cart', function(event) {
            ...
            var id = ele.attr("data-id");
            $.ajax({
                url: '{{ route('remove.from.special_cart') }}',
                method: 'DELETE',
                data: {
                    _token: '{{ csrf_token() }}',
                    id: ele.attr("data-id")
   ...
   <script>
    $('#cart-container').on('click', '.checkout-btn', function(e) {
        ...
        var dateTime = $('input[name="dateTime"]').val(); // Get the value of the date-time input

        if (!dateTime) {
            alert("Please select a date and time before proceeding with the checkout.");
            return; // Stop the process if the date is not selected
        }
        ...
        if (window.confirm(confirmMessage)) {
            window.location.href = "special_sale-product?customer_id=" + (customerId || "") +
    ...
    <script>
        $('#cart-container').on('click', '.unpaid-btn', function(e) {
            ...
            if (window.confirm(confirmMessage)) {
                window.location.href = "special_save-unpaid-product?customer_id=" + (customerId || "") +
                    "&_token={{ csrf_token() }}";
            }
        });
    </script>
